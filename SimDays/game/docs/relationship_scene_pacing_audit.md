...
- **`lena_shoulder_gesture` (MAJOR) is locked out for competent players.** It gates on `hospital_hard_case_pending`, which is only set when `job_performance < 70` at the end of a hospital shift. Players maintaining good performance never see Lena's only physical-touch breakthrough.
...
|---|---|---|---|
| `lena_shoulder_gesture` trigger | `hospital_hard_case_pending` (only set when perf<70) | Set `hospital_hard_case_pending` when `job_performance < 85` OR after `hosp_shifts >= 10` at rank≥1 regardless of performance | Ensures all hospital players can reach Lena's MAJOR breakthrough |
...
| Marcus missed commitment | No commit creation path | Add Marcus invite in `phone_actionable.rpy`: Marcus texts about basketball/run once `marcus_affection>=25`; acceptance creates commitment | Unblocks the dead scene |

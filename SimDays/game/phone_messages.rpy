...
# Shared upcoming-commitments panel — the one place the player can cancel.
# Only used inside the phone Messages app, so it is sized for the phone screen.
screen commitments_list():
    ...
    if _uc:
        frame:
            ...
            vbox:
                spacing 6
                text "Upcoming" font ACT_FONT size 14 color "#9fb6d6"
                for _cm in _uc:
                    ...
                    hbox:
                        xfill True
                        spacing 8
                        text "▸" font ACT_FONT size 12 color ("#5bcafa" if _ca else "#4a6080") yalign 0.5
                        vbox:
                            xfill True
                            spacing 2
                            text _cm["title"] font ACT_FONT size 13 color "#cfe0f5"
                            text (_day_str + "  ·  " + "%02d:00" % _cm["hour"] + "  ·  " + _cm["location"]) font ACT_FONT size 11 color "#4a6080"
                            if _ca:
                                text "Available now" font ACT_FONT size 11 color "#5bcafa"
                            elif _is_today and _hrs_left > 0:
                                text ("In %.0fh" % _hrs_left) font ACT_FONT size 11 color "#9fb6d6"
                        # All eligibility lives in cancel_commitment/_c_active;
                        # upcoming_commitments() already excludes terminal states.
                        textbutton "Cancel":
                            action [Function(cancel_commitment, _cm["id"], _late_cncl),
                                    Function(deliver_due_messages)]
                            text_font ACT_FONT
                            text_size 12
                            text_color "#c06060"
